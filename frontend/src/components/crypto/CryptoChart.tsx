...
export const CryptoChart = ({ symbol }: CryptoChartProps) => {
  ...
  const limit = RANGES.find(r => r.label === range)!.days
  const { data, isLoading, error } = useCryptoChart(symbol, limit)
  ...
  return (
    <div>
      ...
      {!isLoading && (error || !data?.length) && (
        <EmptyState title="No chart data" description="Chart data syncs twice daily at 8am and 8pm" />
      )}
